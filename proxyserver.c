...
void *listener_thread(void *arg) {
    ...
    while (1) {
        ...
        if (strcmp(request->path, GETJOBCMD) == 0) {
            ...
            // Assuming get_work_nonblocking() fetches the highest priority job from the queue
            // If the queue is empty, send QUEUE_EMPTY error
            struct http_request *job = //get_work_nonblocking();
            if (job == NULL) {
            ...
            // Handle file request
            // Assuming add_work() adds the request to the priority queue
            // If the queue is full, send QUEUE_FULL error
            if (!add_work(request)) {
                send_error_response(client_fd, QUEUE_FULL, "Queue is full");
            } else {
                // Acknowledge the addition of the request to the queue
                http_start_response(client_fd, OK);
                http_send_header(client_fd, "Content-Type", "text/plain");
                http_end_headers(client_fd);
                http_send_string(client_fd, "Request added to the queue");
            }
            // Free the request as it's already been added to the queue or rejected
            free(request->method);
            free(request->path);
            free(request);
        }
...
void serve_forever(int *server_fd) {
    pthread_t lisThreads[num_listener]
    // create a socket to listen
    for(int i = 0; i < num_listener; i++) {
        server_fd[i] = socket(PF_INET, SOCK_STREAM, 0);
        if (*server_fd[i] == -1) {
        ...
        int socket_option = 1;
        if (setsockopt(*server_fd[i], SOL_SOCKET, SO_REUSEADDR, &socket_option,
        ...
        if (pthread_create(&threads[i], NULL, listener_thread, &server_fds[i]) != 0) {
    ...
    for(int i = 0; i < num_listener; i++) {
        pthread_join(lisThreads[i], NULL);
    }
